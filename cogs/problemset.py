...
class Problemset(commands.Cog):
    ...
    async def problem(self,ctx,*,args=""):
        ...
        if not tags and not diff:
            ...
            problems = list(pracc_problems.find({ "rating" : {"$gte" : diff-100, "$lte" : diff + 100} }))
        elif not tags :
            ...
            if wild : 
                problems = list(pracc_problems.find({"tags" : { "$all" : tags } , "rating" : diff }))
            else : 
                problems = list(pracc_problems.find({"tags" :  tags , "rating" : diff }))


        if not problems :
            problems = list(pracc_problems.find({"tags" : { "$all" : tags } , "rating" : diff }))
        if not problems : 
            problems = list(pracc_problems.find({"tags" : { "$all" : tags } , "rating" : {"$gte" : diff , "$lte" : diff + 400 } }))
