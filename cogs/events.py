...
class Events(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if str(payload.emoji) == '✅':
            ...
            if f"Account link for {self.client.get_user(payload.user_id).name}" == message.embeds[0].footer.text:
                auth_key = pracc_users.find_one(
                    {'user_id': payload.user_id})['auth_key']
                handle = message.embeds[0].fields[0].value.strip()
                request = requests.get(
                    f"http://codeforces.com/api/user.status?handle={handle}&count=1")
                submittion_id = request.json().get(
                    "result", [{"id": 0}])[0]['id']
                contest_id = request.json().get(
                    "result", [{"contestId": 0}])[0]['contestId']
                request = requests.get(
                    f"http://codeforces.com/contest/{contest_id}/submission/{submittion_id}")
                em = message.embeds[0]
                if str(auth_key) in request.text:
                    em.description = "Verification successful , Account linked :white_check_mark:"
                    pracc_users.update_one({"user_id": payload.user_id}, {
                                           "$set": {"handle": handle}})
                else:
    ...
    async def on_guild_join(self, guild):
        ...
        pracc_servers.update_one({"server_id": guild.id},
                                 {"$set": {"incative": False}})
        return
