...
class Authentication(commands.Cog):
    ...
    async def link(self, ctx, *args):
        if not args or args[0] == "help":
            ...
            em.add_field(
                name=":one:", value="Use !link 'YOUR_CODEFORCES_HANDLE'", inline=False)
            em.add_field(
                name=":two:", value="Get your authentication key", inline=False)
            em.add_field(name=":exclamation:",
                         value="You can regenerate your key if lost", inline=False)
            em.add_field(
                name=":three:", value="Add your key as a comment to one of your recent public submissions to a problem", inline=False)
            em.set_footer(
                text=f"Link command help for {ctx.author.name}", icon_url=ctx.author.avatar_url)
            ...
            em.add_field(name=":white_check_mark:",
                         value="Confirm", inline=True)
            em.add_field(name=":x:", value="Abort", inline=True)
            em.set_footer(
                text=f"Account link for {ctx.author.name}", icon_url=ctx.author.avatar_url)
    ...
    async def link_error(self, ctx, error):
        em = discord.Embed(color=0xF80A0A)
        em.title = "Link Error"
        em.add_field(name="Missing Argument",
                     value="Codeforces Handle", inline=True)
        em.set_footer(
            text=f"Error in Link command for {ctx.author.name}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=em)
