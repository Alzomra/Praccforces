...
class Contests(commands.Cog):
    ...
    async def contest(self,ctx,div=4): 
        ...
        if int(div) < 4 and int(div) > 0 : 
            ...
            p = "A"
            while (True) : 
                test = requests.get(f"https://codeforces.com/contest/{contest['id']}/problem/{p}")
                if test.url !=  f"https://codeforces.com/contest/{contest['id']}/problem/{p}" : 
                    break
                em.add_field(name = f"Problem {p}" , value = f"https://codeforces.com/contest/{contest['id']}/problem/{p}", inline=False )
                p = chr(ord(p)+1)
